    ...
    def update_fairs(self):
        ...
        if self.timestep == 0: pass
            ...
            for month in CONTRACTS:
                ...
                dist = (self.order_book[month]['Best Ask']['Price'] - self.order_book[month]['Best Bid']['Price']) / 3
                self.fairs[month] = round(mid, 2)
                self.spreads[month] = round(dist, 2)
        else:
            for month in CONTRACTS:
                if abs(self.oneSessPos[month]) < 0.5 * abs(self.order_size[month]):
                    self.fairs[month] = round(self.fairs[month] - self.pos[month] * 0.33, 2)
                    self.spreads[month] = self.spreads[month] - 0.1
                    if self.order_size[month] < 20:
                        self.order_size[month] += 1
                if self.oneSessPos[month] >= min(2*self.order_size[month], 75): # we only bought
                    # print("we only buying {}".format(month))
                    # dist = self.order_book[month]['Best Bid']['Price'] + (0.2 * self.order_size[month]) - self.fairs[month]
                    self.fairs[month] = abs(self.order_book[month]['Best Bid']['Price'] + (0.2 * self.order_size[month]) - self.fairs[month])
                    if self.order_size[month] > 5:
                        self.order_size[month] -= 1
                    self.spreads[month] = self.spreads[month] + 0.1
                if self.oneSessPos[month] <= -self.order_size[month]: # we only sell
                    # print("we only selling {}".format(month))
                    # dist = self.order_book[month]['Best Ask']['Price'] - (0.2 * self.order_size[month]) 
                    self.fairs[month] = abs(self.order_book[month]['Best Ask']['Price'] - (0.2 * self.order_size[month]))
                    if self.order_size[month] > 5:
                        self.order_size[month] -= 1
                    self.spreads[month] = self.spreads[month] + 0.1
                ...
                if self.pos[month] > 60: # gotta sell bc we bought too much

                    self.fairs[month] = (0.95 * self.fairs[month])
                    #self.spreads[month] = self.spreads[month] + 1.5
                if self.pos[month] < -60: # gotta buy bc we sold too much
                    self.fairs[month] = (1.05 * self.fairs[month])
                    #self.spreads[month] = self.spreads[month] + 1.5
                
                # elif self.oneSessPos[month] < 0:
                #     self.fairs[month] = self.order_book[month]['Best Ask']['Price'] - (0.1 * self.order_size[month])
                #     if self.order_size[month] > 5:
                #         self.order_size[month] -= 1
                #     self.spreads[month] += 0.2
                # elif self.oneSessPos[month] > 0:
            ...
            print("ONE ROUND POS: {}".format(self.oneSessPos))
